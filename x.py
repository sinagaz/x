###################################################################
#                        Import Module
import json , sys , hashlib , os , time , marshal, getpass, csv, requests, re, mechanize, urllib, time, random
...
def dump_mail():
	...
	try:
		...
		out = open('output/allm_' + n[0].split(' ')[0] +'.txt','w')
...
def getdata():
	...
	if infoz=='1':
	  print '\n'+'[*] Begin Complete DATA Gathering [*]'.center(44) + '\n'
	  search()
	else:
	  main()
...
def search():
	...
	target2 = raw_input("[!] Search by Email1(Enter=All): ")
	target3 = raw_input("[!] Search by Email2(Enter=All): ")    
	...
	if target2 == '': target2 = 'Allz'
		...
		#search()
	#else:
	info(target,target2,target3)

def info(target,target2,target3):
	global a , token
	...
	for i in a['data']:
	  ...
	  if target in  i['name'] or target in i['id'] or target=='Allz':
	   ...
	   if dlyz >= 18  : 
	      dlyz = 0
	      time.sleep(random.randrange(400, 500))
	   ...
	   try:
	     ...
	     katok = 'katok'
	   if target2 in katok or target3 in katok or target2=='Allz':
		#tugas rutin ini adalah test , tandai vuln
		if 'yahoo.com' in katok:
		  ...
		  try:
		    ...
		    lone = 24 - (len(vuln))
		    namel = lone * " "
		    print "\033[36m| " + G + y['email'] + eml + "\033[36m| " + W + vuln + namel + " \033[36m|"
		    continue
	   ...
	   if vulnn=='1' or target2=='Allz':
		#print ' '
		#print ' '
		print G + '[-------- COMPLETE DATA --------]'.center(44)
		outi.write('[-------- COMPLETE DATA --------]\n');
		print W
		try:
			print '[*] Id : '+i['id']
			outi.write('\n[*] Id : '+i['id']+'\n')
